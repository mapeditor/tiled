...
In the order to create an infinite map, make sure the 'Infinite' option is checked in New Map dialog.
...
Except the Bucket Fill Tool, every other tool works exactly in the same way as in the fixed size maps. The bucket fill tool fills the current bounds of that particular tile layer. These bounds get increased upon further painting of that tile layer.
...
In the map properties, you can check/uncheck whether the map should be infinite or not. When converting from infinite to a finite map, the width and height of the final map are chosen on the basis of bounds of all the tile layers.
