...
Layer *MapDocument::addLayer(Layer::TypeFlag layerType)
{
    ...
    case Layer::ObjectGroupType:
        ...
        layer = new ObjectGroup(name, 0, 0, mMap->width(), mMap->height(), mMap->tileWidth(), mMap->tileHeight());
        break;
    case Layer::ImageLayerType:
        name = tr("Image Layer %1").arg(mMap->imageLayerCount() + 1);
        layer = new ImageLayer(name, 0, 0, mMap->width(), mMap->height(), mMap->tileWidth(), mMap->tileHeight());
        break;
    }
    Q_ASSERT(layer);
