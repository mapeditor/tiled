 ...
 */
class AutomaticManager: public QObject
{
    Q_OBJECT
...
public:
     ...
     */
    static AutomaticManager *instance();
...
private:
    Q_DISABLE_COPY(AutomaticManager)

    /**
     * Constructor. Only used by the AutomaticMapping manager itself.
     */
    AutomaticManager(QObject *parent);

    ~AutomaticManager();

    static AutomaticManager *mInstance;
