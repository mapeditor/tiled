...
void PropertyBrowser::valueChanged(QtProperty *property, const QVariant &val)
{
    ...
    if (mCustomPropertiesHelper.hasProperty(property)) {
        ...
        if (mMapComponentProperty.contains(property)) {
            const auto componentName = mMapComponentProperty.value(property);
            undoStack->push(new SetComponentProperty(mDocument,
                                                     objects,
                                                     componentName,
                                                         ...
                                                         const QVariant &value)
{
    ...
    QtVariantProperty *property = mCustomPropertiesHelper.createCustomProperty(name, value);
...
/**
 * Adds all properties to the property browser. This includes the built-in
 * properties, custom properties and component properties.
 */
...
void PropertyBrowser::updateCustomProperties()
{
    ...
    // populate custom properties by object type
    if (!objectType.isEmpty()) {
