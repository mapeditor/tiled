...
AutomaticManager *AutomaticManager::mInstance = 0;

AutomaticManager::AutomaticManager(QObject *parent)
    : QObject(parent)
    , mMapDocument(0)
    , mLoaded(false)
...
AutomaticManager *AutomaticManager::instance()
{
    if (!mInstance)
        mInstance = new AutomaticManager(0);

    return mInstance;
}

void AutomaticManager::deleteInstance()
{
    delete mInstance;
    mInstance = 0;
}

void AutomaticManager::autoMap()
{
    if (!mMapDocument)
        return;
...
void AutomaticManager::autoMap(QRegion where, Layer *l)
{
...
void AutomaticManager::autoMapInternal(QRegion where, Layer *setLayer)
{
    if (!mMapDocument) {
...
bool AutomaticManager::loadFile(const QString &filePath)
{
    mError.clear();
    bool ret = true;
...
void AutomaticManager::setMapDocument(MapDocument *mapDocument)
{
    cleanUp();
    if (mMapDocument)
...
void AutomaticManager::fileChanged(const QString &path)
{
    /*
...
void AutomaticManager::fileChangedTimeout()
{
...
void AutomaticManager::setLayerChanged(const QString &setLayer)
{
    mSetLayer = setLayer;
    foreach (AutoMapper *autoMapper, mAutoMappers) {
