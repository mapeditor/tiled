...
{
    if (mSubFormat == Json)
        return tr("JSON map files [Tiled 1.1] (*.json)");
    else
        return tr("JavaScript map files [Tiled 1.1] (*.js)");
}
...
{
    return tr("JSON tileset files [Tiled 1.1] (*.json)");
}
...
{
    return tr("JSON template files [Tiled 1.1] (*.json)");
}
